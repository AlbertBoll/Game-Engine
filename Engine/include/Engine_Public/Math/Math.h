...
namespace Math
{
    ...
    inline constexpr float Dot3(float ax,float ay,float az, float bx,float by,float bz)
    { return ax*bx + ay*by + az*bz; }

    inline constexpr void Cross3(float ax,float ay,float az, float bx,float by,float bz,
    ...
    inline constexpr float LenSq3(float x,float y,float z) { return x*x + y*y + z*z; }

    inline constexpr bool Normalize3Safe(float& x,float& y,float& z, float eps = 1e-20f)
    {
    ...
    inline constexpr void OrthoFallbackTangent(float nx,float ny,float nz,
    ...
    inline constexpr void BuildTangentSign(std::vector<PrimVertex>& verts,
